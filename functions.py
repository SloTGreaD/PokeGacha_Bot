...
async def fetch_leaderboard():
    async with AsyncDatabaseConnection(DATABASE_FILE) as cur:
        pokemon_columns = ' + '.join([f"SUM(CASE WHEN {pokemon.lower()} > 0 THEN 1 ELSE 0 END)" for pokemon in POKEMON_LIST])
        query = f"""
        SELECT nickname, ({pokemon_columns}) AS unique_pokemons
        FROM number_of_pokemons
        GROUP BY user_id
        ORDER BY unique_pokemons DESC
        LIMIT 10
        """
        await cur.execute(query)
...
async def count_unique_pokemons(user_id):
    async with AsyncDatabaseConnection(DATABASE_FILE) as cur:
        columns = ', '.join([f"SUM(CASE WHEN {name.lower()} > 0 THEN 1 ELSE 0 END)" for name in POKEMON_LIST])
        query = f"SELECT {columns} FROM number_of_pokemons WHERE user_id = ?"
        await cur.execute(query, (user_id,))
        result = await cur.fetchone()
        return sum(result)  # Суммируем количество уникальных покемонов
