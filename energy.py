...
async def get_food_counts(user_id):
    async with AsyncDatabaseConnection(DATABASE_FILE) as cur:
        query = "SELECT bread, rice, ramen, spaghetti FROM items WHERE user_id = ?"
        await cur.execute(query, (user_id,))
        result = await cur.fetchone()
        return result if result else (0, 0, 0, 0)  # Возвращаем кортеж с нулями, если ничего не найдено
