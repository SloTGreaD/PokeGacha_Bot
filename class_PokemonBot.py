...
class PokemonBot:
    ...
    async def items_buttons(self, chat_id):
        # Получаем количество еды для пользователя
        bread_count, rice_count, ramen_count, spaghetti_count = await energy.get_food_counts(chat_id)

        # Создаем кнопки с количеством еды
        markup = types.InlineKeyboardMarkup()
        button_bread = types.InlineKeyboardButton(f'🍞 Bread ({bread_count})', callback_data='check_bread')
        button_rice = types.InlineKeyboardButton(f'🍚 Rice ({rice_count})', callback_data='check_rice')
        button_ramen = types.InlineKeyboardButton(f'🍜 Ramen ({ramen_count})', callback_data='check_ramen')
        button_spaghetti = types.InlineKeyboardButton(f'🍝 Spaghetti ({spaghetti_count})', callback_data='check_spaghetti')

        # Добавляем кнопки в разметку
        markup.row(button_bread, button_rice)
        markup.add(button_ramen, button_spaghetti)
    ...
    async def profile_button(self, chat_id):
        captured_pokemons = await functions.count_unique_pokemons(chat_id)
        ...
        profile_text = (
            ...
            f"Nickname: {nickname}\n\n"
            f"Level: ???\n\n"
            f"Captured Pokemons: {captured_pokemons}/151"
        )
        await bot.send_message(chat_id, profile_text, reply_markup=markup)
    
    async def send_leaderboard(self, chat_id):
        leaderboard_data = await functions.fetch_leaderboard()
        markup = await under_keyboard_class.profile_menu2()
        leaderboard_message = "🏆 Pokemon Leaderboard 🏆\n\n"
        for index, (nickname, unique_pokemons) in enumerate(leaderboard_data, start=1):
            leaderboard_message += f"{index}. {nickname} - {unique_pokemons}/151\n"
        await bot.send_message(chat_id, leaderboard_message, reply_markup=markup)
